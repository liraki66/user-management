...
import {
    clearReports,
    getSamatWithdrawalsFromBankDebtors, getSamatWithdrawalsFromBankDebtorsDocument,
...
const ReportDebtorsWithdraw = (props) => {
    ...
    const reportList = useSelector(state => state.reports.samatWithdrawalsFromBankDebtors)
    ...
    const columns = [
        ...
        {
            title: 'نام سرفصل',
            dataIndex: 'BankDebtorsName',
        },
        {
            title: 'کد سرفصل',
            dataIndex: 'BankDebtorsCode',
        },
        {
            title: 'نوع بازار',
        ...
        },
        {
            ...
            dataIndex: 'status',
            render: (text, record, index) => {
                if (record.status !== "SettlementNotDone") {
                    return <div className={'auto-settlement-successful'}>{record.statusFa}</div>
                } else {
                    return <div className={'auto-settlement-unsuccessful'}>{record.statusFa}</div>
                }
            },
    ...
    useEffect(() => {
        if (brokerInfo.marketType) {
            ...
            setTimeout(() => {
                dispatch(getSamatWithdrawalsFromBankDebtors(data)).then(res => {
    ...
    const downloadCSV = () => {
        if (brokerInfo.marketType) {
            dispatch(getSamatWithdrawalsFromBankDebtorsDocument({
                body: {...brokerInfo, fromDate: brokerInfo.fromDate, toDate: brokerInfo.toDate},
                param: 'csv',
                name: 'گزارش برداشت های انجام شده از سرفصل بدهکاران'
            }))
        } else {
            notifyMarket()
        }
    ...
    const printPdf = () => {
        if (brokerInfo.marketType) {
            ...
            request.post(GetSamatWithdrawalsFromBankDebtors, data, {baseURL: BrokerSettlementBaseUrl}).then(res => {
                printJS({
                    ...
                    properties: [
                        ...
                        {field: 'brokerCode', displayName: 'نام سرفصل'},
                        {field: 'brokerCode', displayName: 'کد سرفصل'},
                        {field: 'marketType', displayName: 'نوع بازار'},
                        {field: 'contractNumber', displayName: 'شماره قرارداد'},
                        {field: 'settlementTypeFa', displayName: 'نحوه تسویه'},
                        {field: 'amount', displayName: 'مبلغ برداشت شده از سرفصل(ریال)'},
                        {field: 'penaltyAmount', displayName: 'جریمه دیرکرد(ریال)'},
                        {field: 'trackerId', displayName: 'شماره پیگیری'},
                        {field: 'dateTime', displayName: 'تاریخ'},
                        {field: 'statusFa', displayName: 'وضعیت'},
            ...
            `,
                    ...
                    *{direction: rtl;font-family:B Nazanin, B Zar;}`,
                    header: `
                        <div style="text-align: center;margin-bottom: 1rem;direction: rtl;font-weight: bold">گزارش برداشت های انجام شده از سرفصل بدهکاران</div>
    ...
    return (
        <div className={'inquiry-content-container'}>
            ...
            <Row gutter={[24, 24]} style={{marginBottom: '3rem'}}>
                ...
                <Col lg={4} xxl={3}>
                    <CustomButton type={'primary'} title={'گزارش جامع'} onClick={getBankDebtorsReports}/>
                </Col>
            </Row>
...
export default ReportDebtorsWithdraw