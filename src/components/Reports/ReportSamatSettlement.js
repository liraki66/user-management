...
import {getBrokersList, getBrokersListWithPagination} from "../../redux/slices/broker";
...
const ReportSamatSettlement = (props) => {
    ...
    const reportList = useSelector(state => state.reports.transferBrokerToSamat)
    ...
    useEffect(() => {
        if (brokerInfo.marketType) {
            ...
            const data = {
                request: {
                    ...brokerInfo,
                    fromDate: brokerInfo.fromDate.includes(' 00:00:00') ? brokerInfo.fromDate : brokerInfo.fromDate + ' 00:00:00',
                    toDate: brokerInfo.toDate.includes(' 00:00:00') ? brokerInfo.toDate : brokerInfo.toDate + ' 00:00:00'
                },
                pageNumber: pagination.current - 1,
                take: pagination.pageSize
            }
            setTimeout(() => {
                dispatch(getTransferBrokerToSamat(data)).then(res => {
    ...
    const getSamatSettlementReports = () => {
        if (brokerInfo.marketType) {
            setLoading(true)
            const data = {
                request: {
    ...
    const downloadCSV = () => {
        if (brokerInfo.marketType) {
            dispatch(getTransferBrokerToSamatDocument({
                body: {
                    ...brokerInfo,
                    fromDate: brokerInfo.fromDate.includes(' 00:00:00') ? brokerInfo.fromDate : brokerInfo.fromDate + ' 00:00:00',
                    toDate: brokerInfo.toDate.includes(' 00:00:00') ? brokerInfo.toDate : brokerInfo.toDate + ' 00:00:00'
                },
                param: 'csv', name: 'گزارش تسویه بدهی با ذینفع'
            }))
        } else {
            notifyMarket()
        }
    ...
    const downloadExcel = () => {
        if (brokerInfo.marketType) {
            dispatch(getTransferBrokerToSamatDocument({
                ...
                param: 'xlsx', name: 'گزارش تسویه بدهی با ذینفع'
            }))
        } else {
            notifyMarket()
        }
    ...
    const printPdf = () => {
        if (brokerInfo.marketType) {
            ...
            request.post(GetTransferBrokerToSamat, data, {baseURL: BrokerSettlementBaseUrl}).then(res => {
                printJS({
            ...
            `,
                    ...
                    *{direction: rtl;font-family:B Nazanin, B Zar;}`,
                    header: `
                        <div style="text-align: center;margin-bottom: 1rem;direction: rtl;font-weight: bold">گزارش تسویه بدهی با ذینفع</div>
    ...
    return (
        <div className={'inquiry-content-container'}>
            ...
            <Row gutter={[24,24]} style={{marginBottom: '3rem'}}>
                ...
                <Col lg={4} xxl={3}>
                    <CustomButton type={'primary'} title={'گزارش جامع'} onClick={getSamatSettlementReports}/>
                </Col>
            </Row>
...
export default ReportSamatSettlement